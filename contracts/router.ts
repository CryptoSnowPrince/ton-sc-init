...
export function data(params: { isLocked: boolean; adminAddress: Address; LPWalletCode: Cell; poolCode: Cell; LPAccountCode: Cell }): Cell {
  return beginCell()
    ...
    .storeRef(beginCell().storeUint(0, 64).storeUint(0, 64).storeAddress(null).storeRef(beginCell().endCell()).endCell())
    .endCell();
}

export function setFees(params: { jetton0Address: Address; jetton1Address: Address; newLPFee: BN; newProtocolFee: BN; newRefFee: BN; newProtocolFeeAddress: Address }): Cell {
  return beginMessage({ op: new BN(0x355423e5) })
    ...
    .storeRef(beginCell().storeAddress(params.jetton0Address).storeAddress(params.jetton1Address).endCell())
    .endCell();
}
...
export function cancelCodeUpgrade(): Cell {
  return beginMessage({ op: new BN(0x357ccc67) }).endCell();
}

export function cancelAdminUpgrade(): Cell {
  return beginMessage({ op: new BN(0xa4ed9981) }).endCell();
}

export function finalizeUpgrades(): Cell {
  return beginMessage({ op: new BN(0x6378509f) }).endCell();
}
...
export function lock(): Cell {
  return beginMessage({ op: new BN(0x878f9b0e) }).endCell();
}

export function unlock(): Cell {
  return beginMessage({ op: new BN(0x6ae4b0ef) }).endCell();
}

export function collectFees(params: { jetton0Address: Address; jetton1Address: Address }): Cell {
...
export function payTo(params: { owner: Address; tokenAAmount: BN; walletTokenAAddress: Address; tokenBAmount: BN; walletTokenBAddress: Address }): Cell {
  return beginMessage({ op: new BN(0xf93bb43f) })
    ...
    .storeRef(
      beginCell().storeCoins(params.tokenAAmount).storeAddress(params.walletTokenAAddress).storeCoins(params.tokenBAmount).storeAddress(params.walletTokenBAddress).endCell()
    )
    .endCell();
}
...
export function provideLiquidity(params: { jettonAmount: BN; fromAddress: Address; walletTokenBAddress: Address; minLPOut: BN }): Cell {
  return beginMessage({ op: new BN(0x7362d09c) })
    ...
    .storeAddress(params.fromAddress)
    .storeBit(true)
    .storeRef(beginCell().storeUint(new BN(0xfcf9e58f), 32).storeAddress(params.walletTokenBAddress).storeCoins(params.minLPOut).endCell())
    .endCell();
}
